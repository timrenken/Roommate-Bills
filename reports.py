...
class PdfReport:
    ...
    def generate(self, roommate1, roommate2, bill):
        ...
        pdf.cell(w=165, h=30, txt="Period:", border=1)
        pdf.cell(w=150, h=30, txt=bill.period, border=1, ln=1)

        # Insert service label and value
        pdf.cell(w=200, h=30, txt=f"for: {bill.service}", border=1, ln=1)

        # Insert name and due amount of the first roommate
        pdf.set_font(family="Arial", size=12)
        pdf.cell(w=50, h=20, txt=roommate1.name, border=1)
        pdf.cell(w=150, h=20, txt=f"${roommate1_pay}", border=1, ln=1)

        # Insert name and due amount of the second roommate
        pdf.cell(w=50, h=20, txt=roommate2.name, border=1)
        pdf.cell(w=150, h=20, txt=f"${roommate2_pay}", border=1, ln=1)
